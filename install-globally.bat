...
set "DOWNLOAD_URL=https://github.com/beyondbday69/turbovs/releases/latest/download/turbovs.vsix"
set "VSIX_FILE=%SCRIPT_DIR%turbovs.vsix"

powershell -Command "[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; try { (New-Object System.Net.WebClient).DownloadFile('%DOWNLOAD_URL%', '%VSIX_FILE%') } catch { (New-Object System.Net.WebClient).DownloadFile('https://github.com/beyondbday69/turbovs/releases/latest/download/turbovs-1.1.0.vsix', '%VSIX_FILE%') }" >nul 2>&1
if not exist "%VSIX_FILE%" (
    echo [ERROR] Could not download TurboVs VSIX from GitHub.
